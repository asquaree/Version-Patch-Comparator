...
def clone_version(all_version,repository_name):
    ...
    for i in range(len(all_version)):
      ...
      add_emptyfile(path,brch)#adding an empty patch file in the folder
  ...
  for diff in os.listdir(path):
   ...
   # print(brch_diff_len)
    ...
    if(deleted_lines!=0 and added_lines<=1):
      isdeleted=1
    with open(os.path.join(os.getcwd()+'\\Fedora project',csv_name), 'a') as csvfile: #writing the information into the csv for each diff file
      csvwriter = csv.writer(csvfile)
...
def main():  
  ...
  subprocess.run(['git', 'clone',repository_name],cwd=parent_dir )# cloning the repository inside the Fedora project folder
  stdout = subprocess.check_output('git branch -r'.split(),cwd=parent_dir+'\\grub2',) #fetching all branches names
  out = stdout.decode()
  ...
  clone_version(all_version,repository_name)#cloning versions seperately from the repository 
  branches = os.listdir(os.getcwd()+'\\Fedora project\\versions')#fetching all the versions from the versions folder
  branches.sort()
  ...
  diff_folders=os.listdir(os.getcwd()+'\\Fedora project\\versions_diff') #storing all diff folders
  diff_folders.sort()
